...
display_usage() {
  ...
  echo -e "  -a  lineage results from almond."
  echo -e "      Default: <prefix>.csv"
  echo -e "  -l  Log file."
  ...
  echo -e "      Default: ."
  echo -e "  -s  snp-sites vcf table."
  echo -e "      Default: <prefix>.csv"
  echo -e "  -P  pt-sample ampliconstats png file."
  echo -e "      Default: <prefix>.png"
  echo -e "  -b  blind-sample ampliconstats png file."
  echo -e "      Default: <prefix>.png"
  ...
  echo -e "create_almond_report.sh"
  echo -e "This script is part of an SC2 PT analysis pipeline and produces "
...
while getopts ":a:l:m:o:p:s:P:b:t:T:h" opt; do
  case $opt in
    a) analysis=$OPTARG;;
    l) log_file=$OPTARG;;
    m) message=$OPTARG;;
...
if [ -z $log_file ]; then
  log_file="${output_dir}/create_almond_report.log"
fi
...
R -e "Sweave('$sweave_report')" --args  --analysis="$analysis" --message="$message" \
  --snp="$snp" --ptSample="$ptSample" --blindSample="$blindSample"
exit_status=$?
